...
/**
 * Create sitecustomize.py to add bundled packages to sys.path at Python startup.
 * This ensures Python can find packages bundled in the electron app.
 */
function createSiteCustomize(platformDir) {
  console.log(`[download-python] Creating sitecustomize.py...`);

  // Determine Python's built-in site-packages directory
  // python-build-standalone uses consistent paths across platforms
  const pythonLibDir = path.join(platformDir, 'python', 'Lib');
  const pythonLibDirLower = path.join(platformDir, 'python', 'lib', `python${PYTHON_VERSION.split('.').slice(0, 2).join('.')}`);

  // Check which directory exists (Lib vs lib)
  let sitePackagesDir;
  if (fs.existsSync(pythonLibDir)) {
    sitePackagesDir = path.join(pythonLibDir, 'site-packages');
  } else if (fs.existsSync(pythonLibDirLower)) {
    sitePackagesDir = path.join(pythonLibDirLower, 'site-packages');
  } else {
    throw new Error(`Could not find Python Lib directory in ${platformDir}`);
  }
  ...
  // This file is automatically executed by Python on startup before any imports
  const sitecustomizeContent = `"""
Auto-generated sitecustomize.py for Auto-Claude bundled Python runtime.

This file is automatically executed by Python at startup (before any imports)
to add the bundled packages directory to sys.path.

Generated by: apps/frontend/scripts/download-python.cjs
"""
...
# In packaged Electron app, packages are bundled separately from Python installation
bundled_packages = os.path.join(os.path.dirname(sys.executable), '..', 'python-site-packages')

if os.path.exists(bundled_packages):
    bundled_packages_abs = os.path.abspath(bundled_packages)
    if bundled_packages_abs not in sys.path:
        sys.path.insert(0, bundled_packages_abs)
`;

  const sitecustomizePath = path.join(sitePackagesDir, 'sitecustomize.py');
  fs.writeFileSync(sitecustomizePath, sitecustomizeContent, 'utf-8');

  console.log(`[download-python] Created sitecustomize.py at: ${sitecustomizePath}`);

  return sitecustomizePath;
}
...
async function downloadPython(targetPlatform, targetArch, options = {}) {
  ...
  if (needsPythonDownload) {
    ...
    // Create sitecustomize.py to add bundled packages to sys.path
    createSiteCustomize(platformDir);
  } else {
    // Python already exists - ensure sitecustomize.py is present
    // (may be missing if Python was downloaded before this feature was added)
    try {
      const pythonLibDir = path.join(platformDir, 'python', 'Lib');
      const pythonLibDirLower = path.join(platformDir, 'python', 'lib', `python${PYTHON_VERSION.split('.').slice(0, 2).join('.')}`);

      let sitecustomizePath;
      if (fs.existsSync(pythonLibDir)) {
        sitecustomizePath = path.join(pythonLibDir, 'site-packages', 'sitecustomize.py');
      } else if (fs.existsSync(pythonLibDirLower)) {
        sitecustomizePath = path.join(pythonLibDirLower, 'site-packages', 'sitecustomize.py');
      }

      if (sitecustomizePath && !fs.existsSync(sitecustomizePath)) {
        console.log(`[download-python] sitecustomize.py missing, creating...`);
        createSiteCustomize(platformDir);
      }
    } catch (err) {
      console.warn(`[download-python] Warning: Could not check/create sitecustomize.py: ${err.message}`);
    }
  }
